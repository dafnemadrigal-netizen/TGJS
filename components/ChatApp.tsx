...
export default function ChatApp({ user }: { user: User }) {
  ...
  async function sendMessage() {
    ...
    const userContent = text || '(Ver archivos adjuntos)'
    ...
    <div style={s.root}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerRight}>
          ...
          {/* User chip */}
          <div style={s.userChip} onClick={() => supabase.auth.signOut()}>
            <div style={s.userAvatar}>{initials}</div>
            <div>
              <div style={{ fontSize: 12, color: 'var(--text)' }}>{profile?.name}</div>
              <div style={{ fontSize: 10, color: 'var(--muted)' }}>{profile?.country || 'Sin país'} · Cerrar sesión</div>
            </div>
          </div>
        </div>
      </header>
          ...
          <aside style={s.sidebar}>
            ...
            <div style={s.sideSection}>
              ...
              {[['', '🌎', 'Todos'], ['Nicaragua', '🇳🇮', 'Nicaragua'], ['Panamá', '🇵🇦', 'Panamá'], ['El Salvador', '🇸🇻', 'El Salvador']].map(([val, flag, label]) => (
                <div key={val} style={{ ...s.chip, ...(activeCountry === val ? s.chipActive : {}) }}
            ...
            {messages.length === 0 && (
              <div style={s.welcome}>
                ...
                <p style={s.welcomeDesc}>
                  Soy tu asesor estratégico de personas para AMPM CAM. Analizo problemas operativos
                  usando Good Jobs Strategy, los libros de Zeynep Ton y los casos Harvard.
                  Puedo leer imágenes, Excel y PDFs.
                </p>
                <div style={s.chipRow}>
                  {['Alta rotación en tiendas', 'Cobertura de turnos', 'Roadmap Good Jobs', 'Mala experiencia del cliente', 'Liderazgo en tienda', 'Sobrecarga operativa'].map(c => (
          ...
          <div style={s.inputArea}>
            ...
            {usage.count >= usage.limit && (
              <div style={s.limitBanner}>
                🚫 Has alcanzado el límite de {usage.limit} consultas este mes. Se restablecerá el 1 del próximo mes.
              </div>
            )}
            ...
            <div style={s.inputWrapper}>
              ...
              <input ref={fileInputRef} type="file" style={{ display: 'none' }}
                accept="image/*,.xlsx,.xls,.csv,.pdf,.txt,.md"
                multiple onChange={e => e.target.files && handleFileUpload(e.target.files)} />
              <textarea
                ref={textareaRef}
                ...
                rows={1}
              />
              <button style={{ ...s.sendBtn, opacity: loading || (!input.trim() && pendingFiles.length === 0) ? 0.4 : 1 }}
                onClick={sendMessage} disabled={loading || usage.count >= usage.limit}>↑</button>
            </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  sideLabel: { fontSize: 10, fontWeight: 600, letterSpacing: 1.5, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 10 },
  chip: { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 7, background: 'var(--surface2)', border: '1px solid var(--border)', cursor: 'pointer', fontSize: 12, marginBottom: 5, transition: 'all .15s' },
  ...
  quickBtn: { background: 'none', border: '1px solid var(--border)', borderRadius: 7, padding: '8px 10px', color: 'var(--muted)', fontFamily: 'DM Sans, sans-serif', fontSize: 11.5, cursor: 'pointer', textAlign: 'left', lineHeight: 1.4, width: '100%', marginBottom: 5 },

  // Chat
  chatArea: { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  messages: { flex: 1, overflowY: 'auto', padding: '28px 36px', scrollBehavior: 'smooth' },

  // Welcome
  welcome: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center', padding: 40 },
  ...
  chipRow: { display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 560 },
  ...
  msgRole: { fontSize: 11, fontWeight: 600, letterSpacing: 0.8, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 7 },
  ...
  limitBanner: { background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: 'var(--red)', marginBottom: 10, textAlign: 'center' },
  ...
  textarea: { flex: 1, background: 'none', border: 'none', color: 'var(--text)', fontFamily: 'DM Sans, sans-serif', fontSize: 14, lineHeight: 1.5, resize: 'none', outline: 'none', minHeight: 22, maxHeight: 150 },
  ...
  inputHint: { fontSize: 11, color: 'var(--muted)', marginTop: 7, textAlign: 'center' },
}
